...
#'
#' @description This function tries to extract the version number
#' within a file. There are a few pitfalls:
#'
...
#' @param latest a Boolean, if TRUE (default) only the latest version is returned
#'
...
get_version <- function(file, latest = TRUE) {
  ...
  # replace punctuation . / - between numbers with "x" to keep them in the next step
  ...
  version_punct_clean <- grep(pattern = "[[:digit:]]", version_punct, value = TRUE)
